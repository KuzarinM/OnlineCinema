<script>
	...
	export default{
		...
		computed:{
			cssVars(){
				return{
					'--image-url' : 'url("'+`${this._urlPrefix}/image/background/${this.myObject.name}`+'")',
				}
			}
		},
		methods:{
			...
			async downloadSeason(sid, sname){
				const urlDownload = `${this._urlPrefix}/seasons/${sid}/download`

				$(`#loadPlace_${sid}`).html('<img src="http://192.168.1.120:5173/src/assets/load.gif" height="40" width="40">')
	...
	<article class="d-flex flex-column justify-content-center align-items-center" :style="cssVars">
		...
		<div class="d-flex flex-column flex-md-row justify-content-center align-items-start mx-3 mw-md-90"><!--Контейнер для страницы-->
				<div class="d-flex flex-column me-2 w-md-30 p-0">
					<img class="panel w-100 m-0 p-0" :src="`${this._urlPrefix}/image/object/${this.myObject.name}`" ><!--Большая картинка справа-->
				</div>
				<div class="d-flex flex-column mw-md-50">
				...
				<table class="table table-striped table-success align-middle m-0">
					...
					<tbody class="table-group-divider" v-for="(item, index) in this.myObject.mySeasons"><!-- Вот этот вот тег со всем содержимым надо повторять -->
						<tr class="table-success" >
							...
							<td >{{item.name}}</td>
							<td class="d-flex">
								<a class="btn btn-success" :href="`${this._urlPrefix}/seasons/${item.id}/download`">Скачать сезон</a>
								...
								<table class="d-none table table-striped table-hover table-borderless table-success align-middle" :id="	`line_${index}`">
									<tbody>
										<tr class="table-success" v-for="episode in item.episodes">
											<td scope="row"></td>
											<td>{{episode.item2}}</td>
											<td><a class="btn btn-success" :href="`${this._urlPrefix}/episodes/${episode.item1}/download`" role="button">Скачать</a></td>
										</tr>
									</tbody>
								</table>
							</td>
